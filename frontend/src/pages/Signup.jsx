  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6">
      <div className="bg-gray-900 border border-gray-800 p-8 rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <p className="text-gray-400 mb-8 text-md font-medium">Join the next generation of virtual trading.</p>

        {error && <div className="bg-red-900/30 border border-red-500/50 text-red-300 p-3 rounded-lg mb-6 text-sm">{error}</div>}
        {success && <div className="bg-green-900/30 border border-green-500/50 text-green-300 p-3 rounded-lg mb-6 text-sm">Account created! Check your email to verify.</div>}
        ...
        <form onSubmit={handleSignup} className="space-y-6">
          ...
          <div>
            <label className="block text-gray-400 text-xs font-bold uppercase mb-2">Create Password</label>
            <input 
              type="password" 
              required
              className="w-full bg-black border border-gray-800 p-3 rounded-lg text-white focus:outline-none focus:border-blue-500 transition-colors"
        ...
        <div className="mt-8 text-center text-md font-medium text-gray-500">
          Already have an account? <Link to="/login" className="text-blue-400 hover:text-blue-300 hover:font-bold">Sign In</Link>
        </div>
      </div>
    </div>
  );
